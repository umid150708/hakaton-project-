 ...
 *
 * Topic gating runs in two code-enforced layers (the model isn't trusted to
 * police its own output):
 *   1. Regex pre-filter catches obvious off-topic asks before any LLM call.
 *   2. The model returns { on_topic, reply }; on on_topic:false we discard
 *      its reply and substitute our own fixed refusal.
 * Providers: Gemini gemini-2.5-flash primary, Groq llama-3.1-8b-instant fallback.
 */
...
// Layer 1: off-topic pre-filter — blatant unrelated asks blocked before any LLM call.
const OFF_TOPIC_PATTERNS: RegExp[] = [
  ...
  // Unambiguous software-dev jargon (e.g. the "hide phone with CSS in the frontend"
  // case). Deliberately narrow: bare words like "api"/"server"/"backend" are NOT
  // here, so a genuine IT-business owner asking about credit/tax isn't blocked —
  // the LLM classifier handles those nuanced cases.
  /\b(devtools|inspect element|css bilan yashir|sql injection|xss|api endpoint|schema cache|localhost|git\s+(commit|push|clone|pull))\b/i,
...
// System prompt: give the model only what it can't know — specific amounts,
// law article numbers, role constraints.
...
function parseChatResult(raw: string): ChatResult {
  try {
    ...
    // Malformed JSON — treat raw text as the reply. Safety net only, not the topic gate.
...
async function callGemini(history: Message[], system: string): Promise<ChatResult> {
  return withGemini(async (genAI) => {
    // gemini-1.5-flash 404s on the current API version. thinkingBudget:0
    // disables hidden reasoning so 2.5-flash answers directly.
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Layer 1 — pre-filter obvious off-topic asks without spending an LLM call.
  ...
  // Gemini primary, Groq fallback if rate-limited.
  try {
  ...
  // Layer 2 — on_topic:false always resolves to our fixed refusal, never the model's reply.
